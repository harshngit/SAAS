...
const paymentStatusFilterOptions = [{ value: 'all', label: 'All' }, ...INVOICE_PAYMENT_STATUSES.map(({ value, label }) => ({ value, label }))]
const invoiceStatusFilterOptions = [{ value: 'all', label: 'All' }, ...INVOICE_STATUSES.map(({ value, label }) => ({ value, label }))]
const customerFilterOptions = [
  { value: 'all', label: 'All Customers' },
  ...Array.from(new Set(salesInvoicesData.map((invoice) => invoice.customerName))).map((name) => ({ value: name, label: name })),
]
...
function SalesInvoicesPanel() {
  ...
  const stats = useMemo(() => getInvoiceStats(salesInvoicesData), [])
  ...
  const filtered = useMemo(() => {
    ...
    return salesInvoicesData.filter((invoice) => {
      ...
      if (invoiceStatus !== 'all' && invoice.invoiceStatus !== invoiceStatus) return false
      if (customer !== 'all' && invoice.customerName !== customer) return false
      if (!term) return true
      return (
  ...
  }, [search, paymentStatus, invoiceStatus, customer])
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          delta={{ trend: 'up', positive: true, label: '18% vs last month' }}
        />
        <StatCard
          icon={IndianRupee}
          iconVariant="success"
          ...
          delta={{ trend: 'down', positive: false, label: '6% vs last month' }}
        />
        <StatCard
          icon={RefreshCw}
          iconVariant="warning"
          ...
          delta={{ trend: 'up', positive: false, label: '12% vs last month' }}
        />
        <StatCard
          icon={Receipt}
          iconVariant="info"
          ...
          delta={{ trend: 'up', positive: true, label: '24% vs last month' }}
        />
      </div>
      ...
      <div className="rounded-[1.25rem] border border-neutral-100 bg-white p-4 shadow-(--shadow-card)">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select
            ...
            value={invoiceStatus}
            onChange={updateFilter(setInvoiceStatus)}
            className="w-44"
            triggerClassName="bg-white"
          />
          <Select
            ...
            value={customer}
            onChange={updateFilter(setCustomer)}
            className="w-52"
            triggerClassName="bg-white"
          />
          ...
          <table className="w-full min-w-6xl text-left text-sm">
            <thead>
              <tr className="border-b border-neutral-100 bg-neutral-50/80">
                ...
                <th className="whitespace-nowrap px-4 py-3.5 text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-neutral-400">Order Ref.</th>
            ...
            <tbody className="divide-y divide-neutral-50">
              {paginated.map((invoice) => (
                <tr key={invoice.id} className="transition-colors hover:bg-primary-50/30">
                  <td className="cursor-pointer whitespace-nowrap px-4 py-3.5 font-medium text-primary-700" onClick={() => navigate(`/admin/invoices/${invoice.invoiceNumber}`)}>
                    {invoice.invoiceNumber}
                  </td>
                  <td className="whitespace-nowrap px-4 py-3.5 text-neutral-800">{invoice.customerName}</td>
                  <td className="whitespace-nowrap px-4 py-3.5 text-neutral-500">{invoice.orderNumber}</td>
                  ...
                  <td className="whitespace-nowrap px-4 py-3.5 text-right text-neutral-600">{formatCurrency(invoice.paidAmount)}</td>
                  <td className="whitespace-nowrap px-4 py-3.5 text-right text-neutral-600">{formatCurrency(invoice.dueAmount)}</td>
                  <td className="whitespace-nowrap px-4 py-3.5">
                    <Badge variant={paymentStatusBadgeVariant(invoice.paymentStatus)} dot>{invoice.paymentStatus}</Badge>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3.5">
                    <Badge variant={invoiceStatusBadgeVariant(invoice.invoiceStatus)}>{invoice.invoiceStatus}</Badge>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3.5">
                    <div className="flex items-center justify-end gap-1 text-neutral-400">
                      <button type="button" onClick={() => navigate(`/admin/invoices/${invoice.invoiceNumber}`)} aria-label="View invoice" className="rounded-lg p-1.5 hover:bg-neutral-100 hover:text-neutral-700">
                        <Eye className="size-4" />
                      </button>
                      <button type="button" aria-label="Print invoice" className="rounded-lg p-1.5 hover:bg-neutral-100 hover:text-neutral-700">
                        <Printer className="size-4" />
                      </button>
                      <button type="button" aria-label="Download invoice" className="rounded-lg p-1.5 hover:bg-neutral-100 hover:text-neutral-700">
                        <Download className="size-4" />
                      </button>
                      <button type="button" aria-label="Share invoice" className="rounded-lg p-1.5 hover:bg-neutral-100 hover:text-neutral-700">
                        <Share2 className="size-4" />
                      </button>
                      <button type="button" aria-label="More actions" className="rounded-lg p-1.5 hover:bg-neutral-100 hover:text-neutral-700">
              ...
              {paginated.length === 0 && (
                <tr>
                  <td colSpan={11} className="px-4 py-10 text-center text-sm text-neutral-400">No invoices match your filters.</td>
                </tr>
              )}
            </tbody>
          </table>
...
const purchaseStatusVariant = { Paid: 'success', Partial: 'warning', Unpaid: 'danger' }
    ...
    <DataTable
      columns={[
        ...
        { key: 'purchaseDate', header: 'Date', sortable: true },
        {
          ...
          header: 'Status',
          sortable: true,
          render: (row) => <Badge variant={purchaseStatusVariant[row.status] || 'neutral'} dot>{row.status}</Badge>,
        },
